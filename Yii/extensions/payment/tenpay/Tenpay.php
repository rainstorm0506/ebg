...
class Tenpay
{
	...
	public static function init($orderSN , $orderMoney , $goodsName = '')
	{
		...
		$reqHandler->setParameter('return_url'			, empty($config['return_url']) ? '' : Yii::app()->createAbsoluteUrl($config['return_url']));
		$reqHandler->setParameter('notify_url'			, empty($config['notify_url']) ? '' : Yii::app()->createAbsoluteUrl($config['notify_url']));
