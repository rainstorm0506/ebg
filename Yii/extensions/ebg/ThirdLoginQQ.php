...
class ThirdLoginQQ
{
	...
	public function __construct()
	{
		if ($this->config = require(Yii::getPathOfAlias('system') . '/../config/thirdLogin.php'))
			$this->config = isset($this->config['ThirdLoginQQ']) ? (array)$this->config['ThirdLoginQQ'] : array();
	...
	{
		$qc = new TencentQC();
		$qc->qq_login($redirect_uri);
	}
}

/*
 * @brief TencentQC类，api外部对象，调用接口全部依赖于此对象
 */
class TencentQC extends TencentOauth
{
...
class TencentOauth
{
	...
	public function __construct()
	{
		$this->recorder		= new TencentRecorder();
		$this->urlUtils		= new TencentURL();
		$this->error		= new TencentErrorCase();
	}
	...
	{
		// -------请求参数列表
		$keysArr		= array ('access_token' => $this->recorder->read('access_token'));
...
class TencentRecorder
{
	...
	public function __construct()
	{
		...
		if($this->inc = require (Yii::getPathOfAlias('system') . '/../config/thirdLogin.php'))
			$this->inc = isset($this->inc['ThirdLoginQQ']) ? (object)$this->inc['ThirdLoginQQ'] : (object)array ();
...
/*
 * @brief TencentErrorCase类，封闭异常
 */
class TencentErrorCase
{
	private $errorMsg;
