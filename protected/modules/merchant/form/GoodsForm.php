<?php
class GoodsForm extends WebForm
{
	public $title , $vice_title , $goods_num , $brand_id , $class_one_id , $class_two_id , $class_three_id , $tag_id;
	public $retail_price , $base_price , $stock , $attrInStock , $weight , $content , $cover , $imgsSet , $jd_id;
	public $copy = false;
	public
		$amount = array() ,
		$userLayer = array() ,
	...
	{
		$rules = array(
			array('title , tag_id , cover , content' , 'required' , 'message'=>'<b>{attribute}</b> 是必填或必选项'),
			array('title' , 'length' , 'min'=>1 , 'max'=>38),
			array('stock , attrInStock', 'numerical' , 'integerOnly'=>true),
			array('brand_id , class_one_id , class_two_id , class_three_id', 'numerical' , 'integerOnly'=>true , 'min'=>1),
			...
			array('amount , userLayer , attrs , attrVal , img , imgGroup , vice_title , imgsSet , jd_id' , 'checkNull'),
		);
	...
	{
		$model = ClassLoad::Only('GoodsMer');/* @var $model GoodsMer */
		if ($model->checkTitle($this->title , (int)$this->getQuery('id') , $this->copy))
