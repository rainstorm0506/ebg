	...
	<main>
		<section class="merchant-content merchant-content-b" id="merchant">
			...
			<nav class="merchant-nav" id="merchantNav">
				<?php echo CHtml::link('待报价' , $this->createUrl('index'), array('class'=>'current'));?>
				<?php echo CHtml::link('已报价' , $this->createUrl('yesOffer'));?>
			</nav>
			<section class="jican-wrap">
				<!-- 搜索框 -->
				<?php $active = $this->beginWidget('CActiveForm', array('id'=>'append-form' , 'htmlOptions'=>array('class'=>'mer-search' , '#'=>'list'))); ?>
					<span>关键字：</span>
					<?php
				...
				<table class="goods-tab">
					...
					<tbody>
						<?php if(isset($noPurchase)):foreach ($noPurchase as $key => $val):?>
						<tr>
							...
							<td class="mc"><?php echo $val['isPrice'] >0 ? '正在报价' : '等待报价';?></td>
							<td class="control">
