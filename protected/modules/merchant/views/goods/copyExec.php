		...
		<?php $active = $this->beginWidget('CActiveForm', array('id'=>'formWrap','enableAjaxValidation'=>true)); ?>
			<ul>
				...
				<li class="class-select">
					<h6><i>*</i>商品分类：</h6>
					<?php
						$form->class_one_id = isset($form->class_one_id) ? (int)$form->class_one_id : (int)$goods['class_one_id'];
						$form->class_two_id = isset($form->class_two_id) ? (int)$form->class_two_id : (int)$goods['class_two_id'];
						$form->class_three_id = isset($form->class_three_id) ? (int)$form->class_three_id : (int)$goods['class_three_id'];
						if ($class)
						{
				...
				<li>
					...
					if ($brand)
					{
						$form->brand_id = isset($form->brand_id) ? (int)$form->brand_id : (int)$goods['brand_id'];
