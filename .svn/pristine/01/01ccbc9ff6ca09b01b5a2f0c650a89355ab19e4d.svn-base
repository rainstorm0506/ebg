...
class Order extends SModels {
	...
	public function getGoodsList($order_sn) {
		...
		if($order_sn)
		{
			...
			// 组装sql 语句
			$sql = "SELECT id, goods_cover, goods_attrs, goods_title, goods_type, goods_vers_num, num, unit_price, goods_id, return_sn, is_evaluate, is_reply_evaluate FROM order_goods {$where} ";
	...
	public function setOrderInfo($post) {
		$logData = $orderData = $orderInfo = array ();
		$currentUser = $this->getUser();$isTrue = $flag = true;
		//判断是否存在该订单
		if($post['order_sn'])
		{
			$logData = array(
				...
				if($post['option_ordersn'] && $userInfo['id'])
				{
					...
					if($flag){
						...
						$logData = array(
							...
							'logs' => isset($post['system']) ? "会员超过时限未支付，系统自动取消了此订单。" : ($orderInfo['is_pay'] ? "企业会员 [ {$userName} ] 通过企业中心平台申请取消该订单，进入等待商家审核退款阶段。" : "企业会员 [ {$userName} ] 通过企业中心平台取消了此订单。"),
							'memo' => '',
							'time' => time(),
						);
