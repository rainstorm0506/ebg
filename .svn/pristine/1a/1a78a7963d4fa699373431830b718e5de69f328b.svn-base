	...
	<div class="box-wrap">
		...
		<table class="mer-tab">
			...
			<tbody>
				...
				<tr>
					<td><a href="<?php echo $this->createFrontUrl(($v['goods_type']==2?'used/intro':'goods/index') , array('id'=>$v['goods_id'])); ?>"  title="<?php echo $v['goods_title'];?>" target="_blank"><img src="<?php echo Views::imgShow($v['goods_cover']); ?>" width="80" height="80"></a></td>
					<td class="tl">
						<a href="<?php echo $this->createFrontUrl(($v['goods_type']==2?'used/intro':'goods/index') , array('id'=>$v['goods_id'])); ?>"  title="<?php echo $v['goods_title'];?>" target="_blank"><?php echo strlen($v['goods_title'])>46 ? mb_substr($v['goods_title'],0, 16,'utf-8')."..." : $v['goods_title'];?></a>
					...
					<td class="gray">
						<?php 
							$goodsAttr = json_decode($v['goods_attrs'],true);
					...
					<td rowspan="<?php echo count($vals['goods'])?>" class="lbor gray">
						...
						<?php if($vals['order_status_id'] == 106 && $vals['pay_type'] == 1):echo CHtml::link('确认收货' , 'javascript:;', array('onclick'=>"orderOptions('".$vals['order_sn']."')", 'class' => 'btn-1 btn-1-7 mb5px'))."<br/>";endif;?>
						<?php if($vals['order_status_id'] == 107 && $vals['is_evaluate'] == 0):echo CHtml::link('评价晒单' , $this->createUrl('oneOrderDetail' , array('oid'=>$vals['order_sn'])), array('class' => 'btn-1 btn-1-7 mb5px'))."<br/>";endif;?>
						<?php
			...
			<thead>
				<tr>
					<th colspan="8">
						...
						<span>订单状态：<em class="mc"><?php if($parentInfo['pay_type'] == 2 && $parentInfo['order_status_id'] == 101)echo '等待备货'; else echo $parentInfo['user_title'];?></em></span>
			...
			<tbody>
				...
				<tr>
					<td>
						<a href="<?php echo $v['goods_type'] ==2 ? '/used/intro?id='.$v['goods_id'] : '/goods?id='.$v['goods_id'];?>" title="<?php echo $v['goods_title'];?>" target="_blank"><img src="<?php echo Views::imgShow($v['goods_cover']); ?>" width="80" height="80"></a>
					</td>
					<td class="tl">
					...
					<td rowspan="<?php echo count($val['goods'])?>" class="lbor gray">
						...
						<?php if($parentInfo['order_status_id'] == 107 && $parentInfo['is_evaluate'] == 0):echo CHtml::link('评价晒单' , $this->createUrl('oneOrderDetail' , array('oid'=>$key)),array('class' => 'btn-1 btn-1-7 mb5px'))."<br/>";endif;?>
						<?php 
					...
					$('.overtimes time').each(function(index,item){
						if($(this).attr('value')>0){
							...
							if( currentSecond >= 0 ){
								...
								$.ajax({
									url:"enterprise.order.ajaxAbolish",
									type:"POST",
