		...
		<?php $active = $this->beginWidget('CActiveForm', array('id'=>'formWrap','enableAjaxValidation'=>true)); ?>
			<ul>
				...
				<li>
					...
					<?php
						...
						echo $active->textField($form , 'goods_num' , array('style' => 'width:40%' ,'class'=>'tbox34' , 'placeholder'=>'如果您不输入商品货号，系统将自动生成一个唯一的货号'));
				...
				<li>
					...
					<?php
						if ($brand)
						{
							$form->brand_id = isset($form->brand_id) ? (int)$form->brand_id :'';
            ...
            <ul id="goodsDefaultSet">
                ...
                <li class="infinite-stock">
                    <h6><i>*</i>库存：</h6>
                    <?php
                    $form->stock = isset($form->stock)&&$form->stock!='' ? (int)$form->stock :0;
                    echo $active->textField($form , 'stock' , array('class'=>'tbox34 int-price' , '_val'=>$form->stock)).'<em style="float:none;">件</em>';
                ...
                <li>
                    <h6><i>*</i>重量：</h6>
                    <?php
                    $form->weight = isset($form->weight)&&$form->weight!='' ? (double)$form->weight :0;
