...
<style>.userCode{text-decoration : underline;color:red}</style>
</head>
<body>
	<!-- header start -->	
		...
		<div>
			...
			<nav>
				<div class="s-wrap">
					...
					<dl>
					<?php
						echo '<dd>'.CHtml::link('集采订单' , $this->createUrl('purchase/index') , array('rel'=>'nofollow')).'</dd>';
						echo '<dd>'.CHtml::link('企业收藏' , $this->createUrl('myCollection/index') , array('rel'=>'nofollow')).'</dd>';
						echo '<dd>'.CHtml::link('企业中心' , $this->createUrl('home/index') , array('rel'=>'nofollow')).'</dd>';
	...
	<?php 
		$user = $this->getUser();
	?>
	<section class="merchant-banner company-banner">
		<div>
			<section class="center-head-info">
				<figure><div><img src="<?php echo Views::imgShow(empty($user['face']) ? 'images/default-face.jpg' : $user['face']);?>" width="120" height="120"></div><em class="growth-level"><?php echo GlobalUser::getUserLayerName($user['exp'],$user['user_type']); ?></em></figure>
				<article>
					<header><h2><?php echo $user['nickname']; ?></h2></header>
					<p><span>我的积分：<?php echo $user['fraction']; ?>分</span><i>|</i><span>我的余额：￥<?php echo $user['money']; ?></span><a class="btn-1 btn-1-8" href="/enterprise/myWithdrawal/showWithdrawal">提现</a></p>
			...
			<aside>
				<?php echo CHtml::link('<i class="l-ico-2"></i><span>收货地址</span>' , $this->createFrontUrl('enterprise/myAddress')); ?>
				<?php echo CHtml::link('<i class="l-ico-1"></i><span>修改登录密码</span>' , $this->createFrontUrl('enterprise/personInfo/showVerity'), array('style'=>'width:85px')); ?>
			</aside>
		</div>
	</section>
	<main>
		...
		<nav class="merchant-subnav">
			<ul id="merchantSubNav">
				...
				<li <?php if($this->leftNavType == 'order'):?>class="current"<?php endif;?>>
					<?php echo CHtml::link('<i><b class="c-ico-1"></b></i><span>企业订单</span>' , $this->createFrontUrl('enterprise/order')); ?>
				</li>
				<li <?php if($this->leftNavType == 'purchase'):?>class="current"<?php endif;?>>
					<?php echo CHtml::link('<i><b class="c-ico-1"></b></i><span>集采订单</span>' , $this->createFrontUrl('enterprise/purchase')); ?>
				</li>
				<li <?php if($this->leftNavType == 'comment'):?>class="current"<?php endif;?>>
					<?php echo CHtml::link('<i><b class="c-ico-7"></b></i><span>我的评价<q>'.($total ? $total : '').'</q></span>' , $this->createFrontUrl('enterprise/comment')); ?>
				</li>
				<li <?php if($this->leftNavType == 'collection'):?>class="current"<?php endif;?>>
					<?php echo CHtml::link('<i><b class="c-ico-2"></b></i><span>我的收藏</span>' , $this->createFrontUrl('enterprise/myCollection')); ?>
				</li>
				<li <?php if($this->leftNavType == 'personInfo'):?>class="current"<?php endif;?>>
					<?php echo CHtml::link('<i><b class="c-ico-3"></b></i><span>密码及登录信息</span>' , $this->createFrontUrl('enterprise/personInfo')); ?>
				</li>
				<li <?php if($this->leftNavType == 'preferential'):?>class="current"<?php endif;?>>
					<?php echo CHtml::link('<i><b class="c-ico-5"></b></i><span>优惠券</span>' , $this->createFrontUrl('enterprise/myPreferential')); ?>
				</li>
				<li <?php if($this->leftNavType == 'exchange'):?>class="current"<?php endif;?>>
					<?php echo CHtml::link('<i><b class="c-ico-6"></b></i><span>兑换的商品</span>' , $this->createFrontUrl('enterprise/myExchange')); ?>
				</li>
				<li <?php if($this->leftNavType == 'myAddress'):?>class="current"<?php endif;?>>
					<?php echo CHtml::link('<i><b class="c-ico-9"></b></i><span>收货地址</span>' , $this->createFrontUrl('enterprise/myAddress'));?>
				</li>
				<li <?php if($this->leftNavType == 'withdrawal'):?>class="current"<?php endif;?>>
					<?php echo CHtml::link('<i><b class="c-ico-9"></b></i><span>提现记录</span>' , $this->createFrontUrl('enterprise/myWithdrawal')); ?>
				</li>
				<li <?php if($this->leftNavType == 'commission'):?>class="current"<?php endif;?>>
					<?php echo CHtml::link('<i><b class="c-ico-11"></b></i><span>推荐提成</span>' , $this->createUrl('commission/index'));?>
				</li>
			</ul>
		</nav>
		<?php endif; ?>
