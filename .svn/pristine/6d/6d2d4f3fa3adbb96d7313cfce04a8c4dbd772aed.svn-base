(function(c) {
	c.extend(c.expr[":"], {nic: function(a, c, d, b) {
		return!(0 <= (a.textContent || a.innerText || "").toLowerCase().indexOf((d[3] || "").toLowerCase()))
	}});
	c.fn.editableSelect = function(a) {
		var g = this.clone(), d = c('<input type="text" autocomplete="off">'), b = c('<ul class="es-list">');
		a = c.extend({}, {filter: !0, effect: "default", duration: "fast", onCreate: null, onShow: null, onHide: null, onSelect: null}, a);
		switch (a.effects) {
		case "default":
		case "fade":
		case "slide":
		...
		({init: function() {
			...
			if ((a.filter ? b.find("li:nic(" + d.val() + ")").hide().size() : 0) == b.find("li").size())
				b.hide();
			else
				switch (a.effects) {
				case "fade":
			...
			if (e + a.outerHeight() >= b.scrollTop() + b.outerHeight() || e <= b.scrollTop())
				d ? b.scrollTop(e + a.outerHeight() - b.outerHeight()) : b.scrollTop(e)
		}, copyAttributes: function(a, b) {
			var e = c(a)[0].attributes, d;
			for (d in e)
				c(b).attr(e[d].nodeName, e[d].nodeValue)
		}, setField: function(b) {
			if (!c(this).is("li:visible"))
				return!1;
			d.val(c(this).text());
			//手动添加隐藏于并把id值添加到input中
			$("input[name='OrderForm[uid]']").val(c(this).attr('value'));
			b.hide();
