...
class GoodsController extends WebController
{
	...
	public function actionIndex()
	{
		...
		#print_r($this->getUnJsonArray($goods['amount_ratio']));exit;
		$goodsAttrs = GlobalGoods::getAttrs($id);
		$this->render('index' , array(
			'goods'			=> $goods,
			'pic'			=> $model->getGoodsPic($id),
			'attrs'			=> $model->getGoodsAttrs($this->getUnJsonArray($goods['attrs']),$goodsAttrs,(int)$goods['class_one_id'],(int)$goods['class_two_id'],(int)$goods['class_three_id']),
			'goodsAttrs'	=> $goodsAttrs,
			'args'			=> $this->getUnJsonArray($goods['args']),
			'jd_price'		=> $jd_price,
			'amountRatio'	=> $this->getUnJsonArray($goods['amount_ratio']),
			'userRatio'		=> $model->getUserRatio($this->getUnJsonArray($goods['user_layer_ratio'])),
			'goodsJoin'		=> $model->getJoinGoods($this->getUnJsonArray($goods['join_goods'])),
			'store'			=> $store,
		));
	}
