	  ...
	  <div class="tabCon">
		...
		<div style="width:99%;">
			<table class="public-table">
				...
				<tbody>
					<?php foreach ($moneyLogs as $monl):?>
						<tr>
							<td><?php echo date('Y-m-d H:m:s',$monl['time']);?></td>
							<td><?php echo '￥'.$monl['money'];?></td>
							<td><?php echo $monl['action_name']; ?></td>
							<td></td>
							<td></td>
						</tr>
					<?php endforeach;if(!$moneyLogs):?>
...
function getData(page,data){  
    ...
    $.ajax({
        type: 'POST', 
        url: "<?php echo $this->createUrl('user/RecommListPage');?>", 
            ...
            if(json.code == 'error'){
	            ...
	            if(list == ''){
	            	...
	            	$.each(list,function(index,array){ //遍历json数据列 
						...
						li+="<td><a href='/supervise/user/detail?id="+array['uid']+"&re_uid="+array['re_uid']+"&start="+start_time+'&end='+end_time+'&key='+key+'&keyword='+keyword+"&page="+page+"'>查看详情</a></td>";
						li+="</tr>";
		            });
