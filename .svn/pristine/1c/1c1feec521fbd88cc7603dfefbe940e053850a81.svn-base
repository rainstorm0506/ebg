	...
	<ul class="form-wraper">
		...
		<li><span>身份证(正面)：</span>
			<a title="点击看大图" class="additem" onclick="$('#ftdialog').dialog('open'); return false;" href="#">
				<img width="80" height="80" src="<?php echo Views::imgShow(empty($merchant['mer_card_front'])?'images/default-face.jpg':$merchant['mer_card_front']); ?>" alt="身份证(正面)">
			</a>
		</li>
		<li><span>身份证(背面)：</span>
			<a title="点击看大图" class="additem" onclick="$('#bkdialog').dialog('open'); return false;" href="#">
			<img width="80" height="80" src="<?php echo Views::imgShow(empty($merchant['mer_card_back'])?'images/default-face.jpg':$merchant['mer_card_back']); ?>" alt="身份证(背面)">
			</a>
		</li>
...
window.onload = function() {
    ...
    var page = "<?php echo empty($condition['page']);?>";
...
function getData(page,data){  
    ...
    $.ajax({
        type: 'POST', 
        url: '/supervise/merchant/RecommListPage', 
        	...
        	if(json.code == 'error'){
	            ...
	            if(list == ''){
	            	...
	            	$.each(list,function(index,array){ //遍历json数据列 
						...
						li+="<td><a href='/supervise/merchant/detail?id="+array['uid']+"&re_uid="+array['re_uid']+"&start="+start_time+'&end='+end_time+'&key='+key+'&keyword='+keyword+"&page="+page+"'>查看详情</a></td>";
						li+="</tr>";
		            });
				}
				$("#pepople").text(total);
