...
button{cursor:pointer}
	...
	<section class="order-wraper">
		...
		<h3 class="tit-1"><span>完成订单备货操作</span></h3>
		<table class="tab-list-1 mb30px">
			...
			<tbody>
				<form id="form">
				<tr>
					<td colspan='2'>订单状态：   <span class="pay-status-green">备货中</span><input type="hidden" name='order_sn' value="<?php echo $order_sn;?>"/></td>
				</tr>
				<!--<tr>
					<td style="width:98px;">商品库存：</td> <td><input type='text' name='express_no' class='send_goods' id="sendnum"/></td>
				</tr>
				-->
				<tr>
				...
				<tr style='margin:5px 0;border-bottom:none'>
					<td style="width:13%;height：100%;" ><button class='submit' style='margin-left:120px'>完成备货</button></td><td><button class="close_option" style="background-color:#d22238;color:white">取消</button></td>
				</tr>
				</form>
			</tbody>
		</table>
...
$(function($){
	...
	$('.submit').click(function(){
		...
		if(flag)
		{
			item = $('#form').serialize()+"&typename=finish_prepare";
			$.ajax({
