 ...
 * @author Jeson.Q
 */
class MyCollectionController extends EnterpriseController
{
	//跳转到企业收藏店铺 列表页
	...
	public function actionDelete()
	{
		...
		if(!empty($typename) && $collection_id){
			$url = $typename == 'store' ? 'enterprise/myCollection/index' : 'enterprise/myCollection/showGoods';
			$type = $typename == 'store' ? 2 : 1;
			$model->deteleCollect ( $collection_id, $id, $type);
			$this->redirect($url);
		}
		$this->redirect('enterprise/myCollection/index');
	}
}