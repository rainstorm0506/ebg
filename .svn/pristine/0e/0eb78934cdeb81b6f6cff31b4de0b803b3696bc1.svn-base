...
#formBox ul li{margin-bottom: 13px;}
			...
			<fieldset class="form-list form-list-36 crbox18-group per-info-form">
				...
				<?php $active = $this->beginWidget('CActiveForm', array('id'=>'append-form','enableAjaxValidation'=>true,'action'=>$this->createUrl('edit'), 'htmlOptions'=>array('class'=>'form-wraper', 'onsubmit'=>'return checkSubmit();'))); ?>
					<ul>
						...
						<li><h6>手机号：</h6><span><?php echo $personData['phone'];?></span><span class="ml20px"><a class="h-c-1" href="javascript:;" onclick="$('.pop-wrap-2,.mask').show();">修改</a></span></li>
						<li><h6> <i> * </i> 用户名：</h6>
						...
						<li><h6> <i> * </i> 真实姓名：</h6>
							<input class="tbox34 tbox34-4" type="text" id="realname" name="User_detail[realname]" value=" <?php if(isset($personData['realname']))echo $personData['realname'];?>">
							<div class="errorMessage" style="display:none"> * 真实姓名不可为空白.</div>
						</li>
						<li>
							<h6><i>*</i>性别：</h6>
							<aside class="dh">
								<label for ="4"><input name="User_detail[sex]" type="radio" value="1" <?php if(isset($personData['sex']) && $personData['sex'] == 1)echo "checked='checked'";if(!isset($personData['sex']))echo "checked='checked'";?> /><i>男</i></label>
								<label for ="5"><input name="User_detail[sex]" type="radio" value="0" <?php if(isset($personData['sex']) && $personData['sex'] == 0)echo "checked='checked'";?> ><i>女</i></label>
							</aside>
						</li>
						<li><h6>生日：</h6>
							<?php if(isset($personData['birthday'])):$dataArr = explode('-', $personData['birthday']);endif;?>
							<select class="sbox36" name="dataYear" id="birthdayYear">
								<option selected>请选年</option>
								<?php for($i=1930;$i<date('Y');$i++):?>
								<option <?php if(isset($dataArr[0]) && $dataArr[0] == $i):echo "selected='selected'";endif;?> value="<?php echo $i;?>"><?php echo $i;?></option>
								<?php endfor;?>
							</select>
							<i class="mlr5px" >年</i>
							<select class="sbox36" name="dataMonth" id="birthdayMonth">
								<option selected>请选月</option>
								<?php for($i=1;$i<=12;$i++):?>
								<option <?php if(isset($dataArr[1]) && $dataArr[1] == $i):echo "selected='selected'";endif;?> value="<?php echo $i;?>"><?php echo $i;?></option>
								<?php endfor;?>
							</select>
							<i class="mlr5px">月</i>
							<select class="sbox36" name="dataDay" id="birthdayDay">
								<?php if(isset($dataArr[2]) && $dataArr[2]):?>
								<option selected value="<?php echo $dataArr[2];?>"><?php echo $dataArr[2];?></option>
								<?php else:?>
								<option selected value="">请选日</option>
								<?php endif;?>
							</select>
							<i class="mlr5px">日</i>
						</li>
						...
						<li><h6>详细住址：</h6><textarea name="User_detail[address]" ><?php if(isset($personData['address']))echo $personData['address'];?></textarea></li>
	...
	<section class="pop-wrap pop-wrap-2" style="height:300px;display: none">
		...
		<fieldset class="form-list form-change-tel" id="formBox">
			<legend>更换手机号</legend>
			<?php $active = $this->beginWidget('CActiveForm',array('action'=>$this->createUrl('/member/personInfo/editPhone',array('s'=>'member')))); ?>
				...
				<ul>
					...
					<?php
						echo $active->textField($form , 'vxcode' , array('ags'=>'vmember','class'=>'tbox38 tbox38-2','placeholder'=>'请输入图形验证码','maxlength'=>6,'style'=>'height:40px'));
	...
	$(function($){
		...
		$('#PersonForm_vxcode').placeholderPlug();
		// ================================== 个人注册表单验证
		var $form = $('#formBox');
		$form.validate({
			rule : {
			...
			submit : function(){
				...
				$.ajax({
					...
					type:"POST",
					async: false,
					data:{tel:tel,codeNum:codeNum,type:'vmember'},
