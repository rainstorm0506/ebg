...
class Governor extends SModels
{
	...
	public function editToPassword(array $post , $uid = 0)
	{
		$this->update('back_governor' , array(
			'password' => GlobalUser::hashPassword($post['password_new2'])
		) , 'id='.$uid);
	}
	...
	public function getUserInfo($account)
	{
		...
		return $this->queryRow("
			SELECT b.*,bb.name AS branch_name
			FROM back_governor AS b
			LEFT JOIN back_governor_branch AS bb ON b.branch_id=bb.id
			WHERE b.account={$this->quoteValue($account)} LIMIT 0,1
		");
	}
	...
	public function checkPasswordCurrent($password)
	{
		...
		return GlobalUser::validatePassword($password, $user['password']);
	}
