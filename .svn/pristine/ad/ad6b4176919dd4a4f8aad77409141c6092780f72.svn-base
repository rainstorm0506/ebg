...
class Cart extends WebModels
{
	...
	{
		if ((int)$gs['final_stock'] !== -999)
		{
			if ($gs['final_stock'] < 1 || $gs['amount'] < 1 || $gs['amount'] > $gs['final_stock'])
				return false;
			...
			if ($gs['type'] == 1)
			{
				if (empty($gs['attrs']))
				{
					if ($this->execute("
						UPDATE goods
						SET stock=stock-{$gs['amount']} , sales=sales+{$gs['amount']}
						WHERE
							id={$gs['id']}
							AND stock>={$gs['amount']}
							AND shelf_id=410
					...
					if ($this->execute("
						UPDATE goods_join_attrs
						SET stock=stock-{$gs['amount']}
						WHERE
							goods_id={$gs['id']}
							AND stock>={$gs['amount']}
							AND attrs_1_unite_code={$this->quoteValue($gs['attrs']['attrs_1_unite_code'])}
							AND attrs_2_unite_code={$this->quoteValue($gs['attrs']['attrs_2_unite_code'])}
							AND attrs_3_unite_code={$this->quoteValue($gs['attrs']['attrs_3_unite_code'])}"
					) < 1)
						return false;
					$this->execute("UPDATE goods SET sales=sales+{$gs['amount']} WHERE id={$gs['id']}");
				}
				//删除商品属性缓存
				GlobalGoods::updateCache($gs['id']);
			}else{
				if ($this->execute("
					UPDATE used_goods
					SET stock=stock-{$gs['amount']} , detail=detail+{$gs['amount']}
					WHERE
						id={$gs['id']}
						AND stock>={$gs['amount']}
						AND shelf_id=1001
	...
	public function userSetDeftAddrs($aid , $uid)
	{
		$transaction = Yii::app()->getDb()->beginTransaction();
		try
		{
			$this->update('user_address' , array('is_default'=>0) , "user_id={$uid}");
			$this->update('user_address' , array('is_default'=>1) , "id={$aid} AND user_id={$uid}");
	...
	public function setUserAddress(array $post , $aid)
	{
		$uid = $this->getUid();
		...
		$columns = array(
			...
			'dict_one_id'	=> (int)$post['dictOneId'],
			'dict_two_id'	=> (int)$post['dictTwoId'],
			'dict_three_id'	=> (int)$post['dictThreeId'],
			'dict_four_id'	=> (int)$post['dictFourId'],
		...
		try
		{
			...
			if ($aid>0)
				$this->update('user_address' , $columns , "id={$aid}");
			else
				$this->insert('user_address' , $columns);
	...
	public function getUserAddressList($uid)
	{
		if ($uid < 1)
			return array();
		return $this->queryAll("SELECT * FROM user_address WHERE user_id={$uid} ORDER BY is_default DESC,id DESC");
	}
}