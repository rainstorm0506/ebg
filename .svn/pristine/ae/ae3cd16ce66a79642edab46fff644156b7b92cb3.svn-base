...
#tab .tabCon div{position:absolute;padding:10px;opacity:0;filter:alpha(opacity=0);}
...
<fieldset class="public-wraper" style="height: 800px;">
		...
		<span style="font-family:'Arial Negreta', 'Arial';font-weight:700;">提成详情</span>
		<span><a class="btn-5" style="display: inline-block;" href="<?php echo $this ->createUrl('merchant/view',array('id'=>$info['id'],'type'=>'推荐奖金','condition'=>$condition));?>">返回</a></span>
		<div style="width:99%;" class="list">
			...
			<?php 
			echo $active->dropDownList($Recommform ,'status' , CMap::mergeArray(array(''=>'订单状态 ') , array('101'=>'待付款','103'=>'待备货','105'=>'备货中','115'=>'备货完成','106'=>'发货中','107'=>'完成')),array('class'=>'sbox32'));
			?>
			<span>下单时间：</span>
			<?php $active->widget ( 'Laydate', array (
					'id' => 'start_time',
					'name' => 'start_time',
			...
			<?php 
			echo $active->dropDownList($Recommform ,'key' , CMap::mergeArray(array(''=>' 搜索关键字 ') , array('real_name'=>'姓名','phone'=>'电话','order_sn'=>'订单号')),array('class'=>'sbox32'));
			?>
			</span>
			...
			<span style="position: absolute;right:10px;">总订单数：<i id="orders"></i></span>
			<?php $this->endWidget(); ?>
...
function getData(page,data){  
    ...
    $.ajax({
        type: 'POST', 
        url: '/supervise/merchant/RecommPage', 
