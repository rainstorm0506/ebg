...
class ThirdLoginWB
{
	...
	public function __construct()
	{
		if ($this->config = require(Yii::getPathOfAlias('system') . '/../config/thirdLogin.php'))
			$this->config = isset($this->config['ThirdLoginWB']) ? (array)$this->config['ThirdLoginWB'] : array();
	...
	{
		header("Location: {$this->loginUrl($callbackURL)}");
	}
	...
	{
		$oAuth	= new SaeTOAuthV2($this->config['appid'] , $this->config['secret']);
		$param	= array(
				'code' 			=> $code,
	...
	{
		return new SaeTClientV2($this->config['appid'] , $this->config['secret'] , $token);
	}
}