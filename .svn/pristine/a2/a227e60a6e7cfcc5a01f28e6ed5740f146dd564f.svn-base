				...
				if($val['goods_type'] == 1)
				{
					if(!empty($goodsAttrs)){
						...
						if(!empty($goodsData) && $goodsData['stock'] != -999){
							...
							$model->execute("UPDATE goods_join_attrs SET stock=stock+{$stock} WHERE goods_id=".(int)$goodsData['id']);
						}
					}else{
						$model->execute("UPDATE goods SET stock=stock+{$val['num']} WHERE id=".(int)$val['goods_id']);
					}
				}else{
						$model->execute("UPDATE used_goods SET stock=stock+{$val['num']} WHERE id=".(int)$val['goods_id']." AND stock != -999");
				}
			}
			return true;
		}
