	...
	<fieldset class="form-list form-list-36 add-goods-form crbox18-group">
		<ul>
								...
								case 1:
									foreach ($attrVal['price'] as $ak => $av)
									{
										...
										echo	'<tr><td class="_t1">'.(empty($_vk[$ak])?'':$_vk[$ak]).'</td>' .
												...
												'<td class="_g_w">'.number_format($attrVal['weight'][$ak],2).' kg</td>'.
									...
									foreach ($attrVal['price'] as $ak => $av)
									{
										foreach ($av as $bk => $bv)
										{
											...
											echo	'<tr><td class="_t1">'.(empty($_vk[$ak])?'':$_vk[$ak]).'</td>' .
													...
													'<td>'.($_xnn==-999?'无限库存':($_xnn.'件')).'</td>'.
													'<td class="_g_w">'.number_format($attrVal['weight'][$ak][$bk],2).' kg</td>'.
													'<td class="_g_ji">'.$_ji.'</td>'.
													'<td class="_g_jp">'.GlobalGoods::jdPrice($_ji).'元</td></tr>';
										}
									}
										...
										foreach ($av as $bk => $bv)
										{
											foreach ($bv as $ck => $cv)
											{
												...
												echo	'<tr><td class="_t1">'.(empty($_vk[$ak])?'':$_vk[$ak]).'</td>' .
														...
														'<td>'.($_xnn==-999?'无限库存':($_xnn.'件')).'</td>'.
														'<td class="_g_w">'.number_format($attrVal['weight'][$ak][$bk][$ck],2).' kg</td>'.
														'<td class="_g_ji">'.$_ji.'</td>'.
														'<td class="_g_jp">'.GlobalGoods::jdPrice($_ji).'元</td></tr>';
											}
										}
			...
			<li class="goods-check">
				<h6>&nbsp;</h6>
				<?php
					echo CHtml::link('编辑商品' , $this->createUrl('goods/modify' , array('id'=>$goods['id'])) , array('id'=>'modifys'));

					if ($goods['shelf_id'] != 410)
						echo CHtml::link('商品上架' , $this->createUrl('goods/check' , array('gid'=>$goods['id'] , 'k'=>1)));
					if ($goods['shelf_id'] != 411)
						echo CHtml::link('商品下架' , $this->createUrl('goods/check' , array('gid'=>$goods['id'] , 'k'=>0)));
				?>
				<q id="checkResult"></q>
			</li>
		</ul>
