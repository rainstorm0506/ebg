<?php
class UserForm extends WebApiForm
{
	...
	// 个人 / 企业模块 - 找回密码
	public function verifyFind()
	{
		if (!Verify::isPhone($this->phone))
			return $this->addError('phone' , '手机号码错误');
		
		if (!$this->checkVxCode('vxCode' , array() , 'find'))
			return false;
		
		if (!$this->checkSmsCode('smsCode' , array() , 'find'))
			return false;
		...
		if (!$model->getUserInfo($this->phone))
			return $this->addError('phone' , '此手机号码没有注册!');
	 ...
	 * @param		array		$args		验证的参数
	 * @param		string		$seeKEY		session key(自定义参数)
	 */
	public function checkSmsCode($key , array $args , $seeKEY = 'sign')
	{
		$seeKEY		= empty($args['seeKEY']) ? $seeKEY : $args['seeKEY'];
		$session	= Yii::app()->session;
		$smsCode	= empty($session['smsCode'][$seeKEY]) ? array() : (array)$session['smsCode'][$seeKEY];
	 ...
	 * @param		string		$seeKEY		session key(自定义参数)
	 */
	public function checkVxCode($key , array $args , $seeKEY = 'sign')
	{
		$seeKEY		= empty($args['seeKEY']) ? $seeKEY : $args['seeKEY'];
		$session	= Yii::app()->session;
		$sessCode	= empty($session['imgCode'][$seeKEY]) ? array() : $session['imgCode'][$seeKEY];
		
		if (empty($sessCode['code']) || $sessCode['code'] != $this->vxCode)
			return $this->addError('vxCode' , '图像验证码错误!');
