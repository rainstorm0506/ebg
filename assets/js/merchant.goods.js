...
{
	var value = attrVal||'' , i = null , name = (inputFormName || 'GoodsForm') + '[' + names.join('][') + ']' , s = '';
	for (i in names)
	{
		if (i<1) continue;
	...
	s = '<input class="tbox28 '+(cls||"")+'" value="'+value+'" name="'+name+'" '+(disabled?'disabled="disabled"':'')+' type="'+(type||"text")+'">';
	if (returnVal === true)
		return [s , value];
	else
		return s;
...
function drawingPicGroup(json)
{
	...
	var
		...
		name = ((inputFormName||'GoodsForm')+'[imgGroup]'),
		index = '',
		_data = {};
...
function drawingTable(attrs)
{
	...
	if (!$.isEmptyObject(attrs))
	{
		...
		cc += '<th>基础价</th><th class="show-price"></th><th>库存(默认无限库存)</th><th>重量</th><th>京东商品id</th></tr></thead><tbody>';
		...
		if (cObj !== null)
		{
				...
				for (bi in bObj)
				{
					for (ci in cObj)
					{
						...
						cc +=	'<tr><td class="_t1">'+aObj[ai]+'</td><td class="_t2">'+bObj[bi]+'</td><td class="_t3">' +
								cObj[ci]+'</td><td class="_g_p">'+_dtInput(['attrVal','price',ai,bi,ci],'text','double-price')+' 元</td>' +
								...
								'</td><td class="_g_w">'+_dtInput(['attrVal','weight',ai,bi,ci],'text','double-price') + ' kg</td>'
								+'<td class="_g_j">'+_dtInput(['attrVal','jd_id',ai,bi,ci],'text','int-price')+'</td></tr>';
					}
				}
			}
		}else if (bObj !== null){
			for (ai in aObj)
			{
				for (bi in bObj)
				{
					...
					cc +=	'<tr><td class="_t1">'+aObj[ai]+'</td><td class="_t2">'+bObj[bi]+'</td>' +
							'<td class="_g_p">'+_dtInput(['attrVal','price',ai,bi],'text','double-price')+' 元</td>' +
							'<td class="show-price"></td>' +
							'<td class="infinite-stock"><a'+(hideText[1]>0||hideText[1]==-999?' class="this"':'')+'>无限库存</a>' +
							hideText[0] + _dtInput(['attrVal','stock',ai,bi],'text','int-price',hideText[1]>0||hideText[1]==-999) +
							'</td><td class="_g_w">'+_dtInput(['attrVal','weight',ai,bi],'text','double-price')+' kg</td>'
							+'<td class="_g_j">'+_dtInput(['attrVal','jd_id',ai,bi],'text','int-price')+'</td></tr>';
				}
			}
		}else{
			for (ai in aObj)
			{
				...
				cc +=	'<tr><td class="_t1">'+aObj[ai]+'</td><td class="_g_p">'+_dtInput(['attrVal','price',ai],'text','double-price')+' 元</td>' +
						'<td class="show-price"></td>' +
						'<td class="infinite-stock"><a'+(hideText[1]>0||hideText[1]==-999?' class="this"':'')+'>无限库存</a>' +
						hideText[0]+_dtInput(['attrVal','stock',ai],'text','int-price',hideText[1]>0||hideText[1]==-999)+'</td>' +
						'<td class="_g_w">'+_dtInput(['attrVal','weight',ai],'text','double-price')+' kg</td>'
						+'<td class="_g_j">'+_dtInput(['attrVal','jd_id',ai],'text','int-price')+'</td></tr>';
			}
		}
		cc += '</tbody>';
...
function postClassArgs(post)
{
	...
	var code = _val = '' , ak = bk = 0 , name = ((inputFormName||'GoodsForm')+'[args]');
...
{
	$('.goods-args>ul').remove();
	var code = '' , child = null , a = i = j = x = 0 , name = ((inputFormName||'GoodsForm')+'[args]');
	for (i in json)
	{
		a = parseInt(i,10) + 1;
...
function calculatePrice(e , val)
{
	...
	if ($('#goodsAttrs>table').is(':visible'))
	{
		...
		$('td.show-price').show().each(function(i , n){
			...
			{
				old = old.toFixed(2);
				$(n).html('* ' + val + '% = ' + (old*val/100).toFixed(2) + '元');
			}
		});
...
$(document).ready(function(){
	var formName = inputFormName || 'GoodsForm';
	//拉动
	$('.js-drag').dragPlug({
		dir:'x',
		cursor:'pointer',
	...
	//数量及价格 第一列输入框
	.on('change' , '.per-wrap input.as' , function()
	{
		var
			ix = $(this).index('.per-wrap input.as') ,
			no = $(this).siblings('input.ae') ,
			nv = parseInt(no.val()||0 , 10) ,
			tv = 0;

		if (ix > 0)
		{
			tv = parseInt($('.per-wrap input.ae:eq('+(ix-1)+')').val() || 0 , 10) + 1;
			$(this).val(tv);
	...
	//数量及价格 第二列输入框
	.on('change' , '.per-wrap input.ae' , function()
	{
		var
			ix = $(this).index('.per-wrap input.ae') ,
			tv = parseInt($(this).val()||0 , 10) ,
			po = $(this).prevUntil('input.as') ,
			pv = parseInt(po.val()||0 , 10);

		if (tv <= pv)
			$(this).val(pv + 1);

		if (ix < 4)
			$('.per-wrap input.as:eq('+(ix+1)+')').val(tv+1).change();
	})
	//百分比框
	.on('change' , '.per-wrap input.tc' , function()
	{
		var
			v = parseInt($(this).val()||0 , 10),
			v = v > 100 ? 100 : v,
			v = v < 0 ? 0 : v,
			ao = $(this).siblings('.js-drag-range:eq(0)'),
			dd = ao.children('dd.js-drag'),
			x = ((ao.width()||0) - (dd.width()||0)) / 100 * v;
		$(this).val(v);
		dd.attr('value' , v).css({'left':x});
		calculatePrice($(this) , v);
	})
	//商品分类 select
			...
			if (!$.isEmptyObject(json))
			{
				...
				for (i in json)
				{
					if (!$.isEmptyObject(json[i].child))
					{
						imgsSet = imgsSet || i;
						data = json[i].child;
						code += '<div><span>'+json[i].title+'</span><label class="attrs-img"><input type="radio" value="'+i+'" name="'+formName+'[imgsSet]" '+(imgsSet==i?'checked="checked"':'')+'><i>图片组</i></label>';
						for (j in data)
						{
	...
	//数量及价格 & 会员及价格 [收起 , 展开]
	.on('click' , '.pack-more' , function()
	{
		if ($(this).text() == '展开')
		{
			$(this).text('收起').parent('div').nextAll('div').show();
		}else{
			$(this).text('展开').parent('div').nextAll('div').hide();
		}
	})
	//商品参数 添加一栏
	...
	//选择图片组跟随的属性
	.on('change' , '.attrs-img>:radio' , function()
	{
		var pic = $('.attrs-img>:checked').val();
		drawingPicGroup(pic && !$.isEmptyObject(attrsData[pic]) ? attrsData[pic] : {});
	})
	...
	.on('submit' , function()
	{
		...
		if (parseFloat($.trim($('#'+formName+'_retail_price').val())||0) <= 0)
			_error[4] = '请填写正确的零售价';
