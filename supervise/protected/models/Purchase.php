		...
		public function outAllByCondition(array $post,$is_split) {
			...
			if(!empty($purchaseList)) {
				foreach ($purchaseList as $key => $val) {
					...
					$data[$key]['create_time']			= !empty($val['create_time'])?date('Y-m-d H:i:s',$val['create_time']):'';
					$data[$key]['price_endtime']		= !empty($val['price_endtime'])?date('Y-m-d H:i:s',$val['price_endtime']):'';
					$data[$key]['wish_receivingtime']	= !empty($val['wish_receivingtime'])?date('Y-m-d H:i:s',$val['wish_receivingtime']):'';
