	...
	<section class="order-wraper">
		...
		<table class="tab-list-1 mb30px">
			...
			<tbody>
				...
				<tr>
					<td style="width:82px;">支付方式： </td> <td style="padding: 10px 7px"><select style="width:156px;" name='pay_port' ><?php if($pay_type == '2'){?><option value='4'>货到付款</option><?php }else{?><option value='1' selected="selected">支付宝支付</option><option value='3'>财付通</option><option value='2'>网银支付</option><option value='5'>邮政汇款</option><?php }?></select></td>
				</tr>
				<tr>
				...
				<tr style='margin:5px 0;border-bottom:none'>
					<?php if($pay_type == 2){?>
					<td style="width:13%;height：100%;" ><button class='submit' style='margin-left:70px;width:130px'>确认用户收货并付款</button></td>
					<td><button class="submit_cancel" style="background-color:#d22238;color:white;width:130px">用户拒收取消订单</button></td>
					<?php }else{?>
...
$(function($){
	//提交相关操作
	$('.submit,.submit_cancel').click(function(){
		...
		if(flag){
			typename = $(this).attr('class') == 'submit' ? 'pay' : 'pay_cancel';
