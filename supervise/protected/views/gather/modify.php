...
<style type="text/css">
    ...
    #one_id{margin-top: 10px;}
</style>
<fieldset class="public-wraper">
	<h1 class="title"><?php echo isset($info['id'])?'编辑':'添加';?> 电脑城商铺</h1>
    <?php $active = $this->beginWidget('CActiveForm', array('id'=>'append-form','enableAjaxValidation'=>true, 'htmlOptions'=>array('class'=>'form-wraper'))); ?>
        <ul>
            ...
            <li>
                ...
                <?php
                    echo ' <p>';
                    $form->computer = $form->computer ? $form->computer : (isset($info['computer'])?$info['computer']:'');
                    echo $active->dropDownList($form , 'computer' , CMap::mergeArray(array(''=>' - 请选择 - '),$computer) , array('class'=>'sbox36'));
                    echo $active->error($form , 'title',array('id'=>'fnone'));
                ?>
            </li>
            <li>
            ...
            <li>
                ...
                <p>
                    <?php
                    $form->storey = $form->storey ? (int)$form->storey : (isset($info['storey'])?(int)$info['storey']:0);
                    echo $active->textField($form , 'storey' , array('class'=>'tbox38 int-price'));
                    ?>
                </p>
                <p><?php echo $active->error($form , 'storey',array('id'=>'fnone1'));?></p>
            </li>
            <li>
                <span><em>*</em>排序：</span>
                <p>
                <?php
                    $form->rank = $form->rank ? (int)$form->rank : (isset($info['rank'])?(int)$info['rank']:0);
                    echo $active->textField($form , 'rank' , array('class'=>'tbox38 int-price'));
                    echo '&nbsp;&nbsp;注：按照数字大小排序，数字越大越靠后！&nbsp;&nbsp;';
                ?>
                </p>
                <p><?php echo $active->error($form , 'rank',array('id'=>'fnone1'));?></p>
            </li>
            <li>
				<h6>&nbsp;</h6>
