...
<fieldset class="public-wraper">
	...
	<ul class="form-wraper">
		<li><span>活动标题：</span><?php
			$activeform->title = !empty($info['title'])?$info['title']:'';
			echo $active->textField($activeform , 'title' , array('class'=>'textbox','maxlength'=>'11'));?></li>
		<li>
			<span>活动类型：</span>
			<?php
			$activeform->type = !empty($info['type'])?$info['type']:'';
		...
		<li>
			<?php if($info['type'] == 1)
			{
				echo '<span>秒购时间点：</span>'.CHtml::dropDownList( 'mgh',($info['attr']['timepoint']),  ($info['attr']['select']) , array (
						'class' => 'sbox32',
				) );
			}
			if($info['type'] == 2)
			{
				echo '<span>折扣率：</span>'.CHtml::textField ('zkl',($info['promotion']), array('placeholder'=>'折扣率','class'=>'tbox7','style'=>"height:30px;"));
			}
			?>
		</li>
		<li><span>会员级别：</span>
			<?php
			$activeform->userexp = !empty($info['userexp'])?$info['userexp']:'';
			...
			?>
			<?php
			$activeform->companyexp = !empty($info['companyexp'])?$info['companyexp']:'';
		...
		<li style="padding-bottom: 10px;"><span>限购次数：</span>
			<i>活动期间每人限购<i><?php echo CHtml::textField ('userlimitce',(!empty($info['userlimitce'])?$info['userlimitce']:''), array('placeholder'=>'活动期间限购','class'=>'tbox20','style'=>"height:30px;"));?>次&nbsp;&nbsp;&nbsp;
			<i>活动期间每天每人限购<i><?php echo CHtml::textField ('userdaylimit',(!empty($info['userdaylimit'])?$info['userdaylimit']:''), array('placeholder'=>'活动期间限购','class'=>'tbox20','style'=>"height:30px;"));?>次
		</li>
		<li><span>时间：</span>
			<ul>
			...
			<?php }else{ ?>
			<?php foreach($info['time'] as $key=>$val){?>
			<li>
			<span style="width:280px;">
				<?php
				$active->widget ( 'Laydate', array (
						...
						'style' => 'width:120px;height:30px;',
						'placeholder'=>'开始时间',
						'value'=>(!empty($val['starttime'])?date('Y-m-d H:i:s',$val['starttime']):''),
				) );
				?><i>—</i><?php
				$active->widget ( 'Laydate', array (
						'id' => 'end',
						...
						'style' => 'width:120px;height:30px',
						'placeholder'=>'结束时间',
						'value'=>(!empty($val['endtime'])?date('Y-m-d H:i:s',$val['endtime']):''),
				) );
				?>
			</span>
			<?php if($key == (count($info['time'])-1)){?>
			<a href="javascript:;" onclick="addactivedate(this);">[+]</a>
			<?php }else{ ?>
		...
		<li>
			...
			<table class='gooditemtb'>
				...
				<tbody id="joingoods">
				<?php if(!empty($info['goods'])){foreach($info['goods'] as $key=>$val){?>
				<tr>
					<td><input type="hidden" name="gids[]" value="<?php echo $val['gid'];?>"><?php echo !empty($val['title'])?$val['title']:'';?></td>
					<td><?php echo !empty($val['stock'])?$val['stock']:'';?></td>
					<td><?php echo CHtml::textField ('nums[]',$val['num'], array('placeholder'=>'数量','class'=>'tbox7','style'=>"height:20px;"));?></td>
					<td><?php echo CHtml::textField ('onece[]',$val['limitnum'], array('placeholder'=>'单次限购数量','class'=>'tbox7','style'=>"height:20px;"));?></td>
					<td><?php echo $val['original_price'];?></td>
					<td><?php echo CHtml::textField ('price[]',$val['price'], array('placeholder'=>'价格','class'=>'tbox7','style'=>"height:20px;"));?></td>
		...
		<li class="appends">
			...
			?></li>
		<li><span>备注：</span><?php echo CHtml::textArea ('remark',(!empty($info['remark'])?$info['remark']:''), array('placeholder'=>'备注','rows'=>6, 'cols'=>50));?></li>
	</ul>
