...
<fieldset class="form-list-34 form-list-34-1 crbox18-group">
	<?php $active = $this->beginWidget('CActiveForm', array('id'=>'formWrap','enableAjaxValidation'=>true)); ?>
		<ul>
			...
			<li>
				<h6><i>*</i>所属商家：</h6>
				<?php if ($used['merchant_id']): 
					$form->merchant_id = empty($form->merchant_id) ? $used['merchant_id'] : (int)$form->merchant_id;
				...
				<table class="user-merchant" style="clear:none;margin:0">
					...
					<tr>
						<?php if($m=GlobalMerchant::getMerchantInfo($used['merchant_id'])):?>
						<td><?php echo $m['mer_no']; ?></td>
						<td><?php echo $m['mer_name']; ?></td>
						<td class="_l"><?php echo $m['store_name']; ?></td>
						<td><?php echo $m['is_self']>0 ? '自营' : '商家'; ?></td>
						<?php endif;?>
					</tr>
				</table>
				<?php endif; ?>
