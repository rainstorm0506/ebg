...
<fieldset class="form-list-34 crbox18-group mt30px">
	<h1 class="title">商品分类 - 设定SEO关键词</h1>
	<?php $active = $this->beginWidget('CActiveForm', array('id'=>'append-form','enableAjaxValidation'=>true, 'htmlOptions'=>array('class'=>'formWrap'))); ?>
		<ul>
			<li class="gcShow">
				<h6><em>*</em> 分类名称：</h6>
				<?php
					$rn = '<span>'.GlobalUsedClass::getClassName($info['root_id']).'</span>';
					$pn = '<span>'.GlobalUsedClass::getClassName($info['parent_id']).'</span>';
					$in = '<span>'.GlobalUsedClass::getClassName($info['id']).'</span>';
					switch ((int)$info['tier'])
					{
						case 1 : echo $in; break;
						case 2 : echo $rn.'<i>-</i>'.$in; break;
						case 3 : echo $rn.'<i>-</i>'.$pn.'<i>-</i>'.$in; break;
					}
				?>
			</li>
			<li>
