<?php
class PointsGoodsForm extends SForm
{
	public $title , $goods_num , $brand_id , $class_one_id , $class_two_id , $class_three_id , $shelf_id , $person , $company , $merchant;
	public $stock , $attrInStock , $weight , $content , $cover , $imgsSet , $points , $remark , $id;
	public
		$amount = array() ,
		$attrs = array() ,
		$attrVal = array() ,
	...
	{
		$rules = array(
			array('title, cover , content' , 'required' , 'message'=>'<b>{attribute}</b> 是必填或必选项'),
			array('attrInStock', 'numerical' , 'integerOnly'=>true),
			array('class_one_id , class_two_id , class_three_id', 'numerical' , 'integerOnly'=>true , 'min'=>1),
			array('title', 'checkTitle'),
			array('class_three_id', 'checkClass'),
			array('brand_id', 'checkBrand'),
			array('goods_num' , 'length' , 'max'=>15),
            array('points','checkPoints'),
			array('goods_num' , 'checkGoodsNum'),
			array('args' , 'checkArgs'),
			array('attrs , attrVal , stock , img , remark , imgsSet , shelf_id , person , company , merchant' , 'checkNull'),
		);
    ...
    public function checkPoints()
    {
        if (!$this->points && $this->points <= 0)
            $this->addError('points' , '请设定正确的 <b>'.$this->getAttributeLabel('points').'</b>');
    }
...
//		$retailPrice = (double)$this->retail_price;
	...
	{
		$model = ClassLoad::Only('PointsGoods');/* @var $model Goods */
